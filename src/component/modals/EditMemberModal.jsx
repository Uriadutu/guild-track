...
const EditMemberModal = ({ setIsOpenModalEdit, getProduk, member }) => {
  const [idMember, setIdMember] = useState("");
  const [namaMember, setNamaMember] = useState("");
  const [loading, setLoading] = useState(false);

  // Isi form dengan data member yang akan diedit
  useEffect(() => {
    if (member) {
      setIdMember(member.id);
      setNamaMember(member.nama);
    }
  }, [member]);
  ...
  const handleEditMember = async (e) => {
    ...
    try {
      // Update data member di Firestore
      const docRef = doc(db, "member", member.docId); // docId adalah ID dokumen Firestore
      ...
      <form onSubmit={handleEditMember}>
        ...
        >
          <div className="flex items-start justify-between p-4 border-b border-gray-200 rounded-t">
            <h3 className="text-xl font-semibold text-gray-700">Edit Member</h3>
            <button
